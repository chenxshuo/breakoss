...
from breakoss.methods import HCoT
...
def main_inference(
        dataset_name: Literal[
            "HarmfulBehaviors", "StrongReject", "HarmBench", "CatQA", "JBBHarmfulBehaviors"] = "HarmfulBehaviors",
    ...
    logger.info(f"Loading transformed dataset {dataset_name} for HCoT method.")
    base_dir = os.path.join(os.path.dirname(__file__), "../transformed_datasets")
    if dataset_name == "StrongReject":
        dataset = StrongReject(data_path=os.path.join(base_dir, "strongreject_dataset_HCoT.csv"))
    elif dataset_name == "HarmfulBehaviors":
        dataset = HarmfulBehaviors(data_path=os.path.join(base_dir, "harmful_behaviors_HCoT.csv"))
    elif dataset_name == "HarmBench":
        dataset = HarmBench(data_path=os.path.join(base_dir, "harmbench_behaviors_text_all_HCoT.csv"))
    elif dataset_name == "CatQA":
        dataset = CatQA(data_path=os.path.join(base_dir, "catqa_english_HCoT.json"))
    elif dataset_name == "JBBHarmfulBehaviors":
        dataset = JBBHarmfulBehaviors(data_path=os.path.join(base_dir, "jbb-harmful-behaviors_HCoT.csv"))
    else:
